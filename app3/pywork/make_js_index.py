# coding=utf-8
""" make_js_index.py for shakuntala  access from MW
...
""" 
 def __init__(self,line,iline,filevol=None):
  ...
  if self.fromv == 0:
   self.fromv = 1
   print('change fromv, tov from 0 to 1')
  x1 =  m_fromv.group(2)
  if x1 == None:
   self.fromvx = ''
  else:
  ...
  if self.tov == 0:
   self.tov = 1
   print('change tov, tov from 0 to 1')
  x2 =  m_tov.group(2)
  if x2 == None:
   self.tovx = ''
  else:
 ...
 def todict(self):
  # 33, 18, 24, 22, 30, 32, 35 (+ 33 18 24 22 30 32 35) = 194
  offsets0 = (0, 33, 19, 24, 22, 30, 32) # 35
  offsets  = (0, 33, 52, 75, 97, 127, 159) # 35
  #offsets = (0,21,35,58,75)
  anka = self.anka
  if anka == 0:
   anka = 1
  offset = offsets[anka - 1]
  fromv = self.fromv  # continuous verses (per pdf)
  tov = self.tov
  v1a = fromv
  v2a = tov
  # verses for anka start at 1  (MW convention for Shakuntala. R,N
  v1 = v1a - offset
  v2 = v2a - offset
  e = {
   #'vol':int(self.vol),
